...
| 2. Публичный каталог Перми | [ ] | Не начат |
| 3. Администрирование и полный auth flow | [ ] | Не начат |
| 4. Очереди, импорт и модерация | [ ] | Не начат |
| 5. Telegram и публикации | [ ] | Не начат |
| 6. MAX | [ ] | Не начат |
| 7. Production hardening и приёмка MVP | [ ] | Не начат |

Текущий scope задания заканчивается после этапа 1. Следующие функциональные этапы не должны незаметно попадать в инфраструктурный foundation.
...
Завершить только этап 1: собрать foundation, установить зависимости, пройти все доступные проверки, обновить его checklist фактическими результатами и остановиться перед реализацией каталога.
